...
public class StringFunctions {
    public static void main(String[] args) throws Exception {
        ...
        Table result1 = tEnv.sqlQuery("SELECT UPPER(`key`),`value` FROM " + table);
        tEnv.toAppendStream(result1, Row.class).print();

        Table result2 = tEnv.sqlQuery("SELECT `key` || 'value' FROM " + table);
        tEnv.toAppendStream(result2, Row.class).print();

        env.execute("Flink string function Java API Skeleton");
    }
}
