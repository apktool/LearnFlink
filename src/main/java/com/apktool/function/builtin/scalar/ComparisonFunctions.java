...
public class ComparisonFunctions {
    public static void main(String[] args) throws Exception {
        ...
        Table result1 = tEnv.sqlQuery("SELECT * FROM " + table + " WHERE key = 'li'");
        tEnv.toAppendStream(result1, KeyValue.class).print();
        tEnv.toAppendStream(result1, Row.class).print();

        Table result2 = tEnv.sqlQuery("SELECT key IN ('li', 'wang') FROM " + table);
        tEnv.toAppendStream(result2, Row.class).print();

        env.execute("Flink comparison function Java API Skeleton");
    }
}
