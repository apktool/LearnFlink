...
public class HashFunctions {
    public static void main(String[] args) throws Exception {
        ...
        Table result1 = tEnv.sqlQuery("SELECT MD5(key) FROM " + table);
        tEnv.toAppendStream(result1, Row.class).print();

        Table result2 = tEnv.sqlQuery("SELECT SHA512(key) FROM " + table);
        tEnv.toAppendStream(result2, Row.class).print();

        env.execute("Flink hash function Java API Skeleton");
    }
}
